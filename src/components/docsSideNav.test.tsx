...
let translate: Signal<boolean> = { value: false } as unknown as Signal<boolean>;
	...
	beforeEach(() => {
		render(
			<DocsContext.Provider
				value={{ title, content, navTop: "0", navSide, theme }}>
				<DocsHeaderContext.Provider
					value={{
						position: positionHandler.position,
						menu,
						translate,
					}}>
					<header>
						<nav
							...
							<span>{title}</span>
							<div
								aria-label="test-hide-nav"
								role="button"
								onClick={positionHandler.setPosition}
