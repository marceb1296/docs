...
export const DocsSideNav = () => {
	...
	return (
		<aside
			aria-label="side-nav"
			style={{
				transform: `translateX(${DocsHeader.position})`,
				right: DocsHeader.translate.value ? "calc(50% - 98ch)" : "0",
			}}
			className="side-nav-doc-container">
			<div className="side-nav-elements">
				{Object.values(Docs.navSide).map(
					({ title, to, childs }, index) =>
						to !== undefined ? (
							<NavAElement
								key={index}
								to={to}
								menu={DocsHeader.menu}
								title={title}
							/>
						) : (
							<NavAElementWithChilds
								key={index}
								menu={DocsHeader.menu}
								title={title}
								childs={childs}
...
const NavAElementWithChilds = ({ title, childs, menu }: IDocsAElement) => {
	...
	const handleClick = () => {
		...
		hanldeList.current.style.height =
			collapseValues.value.dinamic.toString() + "px";
