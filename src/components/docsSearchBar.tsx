...
const SearchBar = () => {
	const { result, queryValue, handleChange } = useTextExtractor();

	const deferredResult = useDeferredValue(queryValue);
	...
	return (
		<>
			{query.length > 0 ? (
				query.map(
					(
						{ title, to, firstMatch, mainMatch, secondMatch },
						index: number
					) => (
						<div key={index}>
							<h3>{title}</h3>
